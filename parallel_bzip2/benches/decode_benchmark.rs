...
fn bench_decode(c: &mut Criterion) {
    ...
    if !input_path.exists() {
        ...
        Command::new("dd").args(&["if=/dev/urandom", "of=../test_bench.bin", "bs=1M", "count=10"]).status().unwrap();
        Command::new("bzip2").args(&["-k", "-f", "../test_bench.bin"]).status().unwrap();
        // Rename to expected path for simplicity in this script context, or just use it
    }
    let bench_file = if input_path.exists() { input_path } else { Path::new("../test_bench.bin.bz2") };
    ...
    group.bench_function("parallel_bzip2", |b| {
        b.iter(|| {
            let mut decoder = Bz2Decoder::new(data.clone());
